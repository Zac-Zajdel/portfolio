import { getPagesUnderRoute } from "nextra/context";
import Image from "components/image";

const RecentPosts = () => {
  const threeRecentBlogs = getPagesUnderRoute("/blog").slice(0, 3);
  return (
    <>
      <h1 className="font-bold text-3xl md:text-5xl tracking-tight mt-8 text-gray-900 dark:text-gray-100">
        Recent Posts
      </h1>
      <div className="my-10 flex flex-wrap justify-center gap-x-7 gap-y-10">
        {threeRecentBlogs.map((page, index) => {
          return (
            <div key={index}>
              <a
                className="flex cursor-pointer flex-col overflow-hidden w-[250px] rounded-xl border border-solid dark:bg-black hover:border-gray-200 no-underline"
                href={page.route}
              >
                <Image
                  src="/static/me.jpeg"
                  alt="Article Logo"
                  width={200}
                  height={200}
                  className="drag-none h-[164px] w-full object-cover"
                />
                <div className="flex grow flex-col p-5">
                  <h2 className="m-0 text-lg font-bold leading-7 text-white dark:text-gray-200 line-clamp-3">
                    {page.meta.title}
                  </h2>
                  <p className="text-white dark:text-gray-200 mb-6 text-xs leading-4 line-clamp-3 overflow-hidden text-ellipsis">
                    {page.frontMatter.description}
                  </p>
                  <div className="mt-auto text-xs">
                    <span className="font-bold text-white dark:text-gray-100">
                      Zac Zajdel
                    </span>
                    <span className="text-white dark:text-gray-100">
                      <span className="select-none"> • </span>
                      {page.frontMatter.date}
                    </span>
                  </div>
                </div>
              </a>
            </div>
          );
        })}
      </div>
    </>
  );
};

export default RecentPosts;
